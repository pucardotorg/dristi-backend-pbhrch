...
| `casemanagement` | 217 | **done** | `case-lifecycle` | `casemanagement` | PR #101 merged to `monolith/main` as `80c708c87` on 2026-05-21 (auxiliary PR #102 `d654c8644` "added mdms url" absorbed into the branch pre-merge). **C1** `7f9b82818` = structural lift of 198 main + 19 test files; 13 protected dups deduped (Document/WorkflowObject/MdmsUtil/IdgenUtil/WorkflowUtil/FileStoreUtil/UserUtil/UrlShortenerUtil/ResponseInfoFactory/Producer/KafkaProducerService/ServiceRequestRepository/AuditDetails); 8 peer-subdomain stereotypes uniquified per Rule 41; Phase 8 lifted commons-codec 1.15 + jsoup 1.17.2 + httpclient 4.5.14 + okhttp 4.12.0; Flyway timestamp normalize (`V202400714144420 → V20240714144420`); Rule 26 pdfbox 3.x adapt (`PDDocument.load(File) → Loader.loadPDF(File)`); Tier 3 dropped orphan service-local `IdgenUtil` (5-arg, 0 callers — canonical 6-arg already in `dristi-common`). **C2** `f06ed6a6c` = 57 contract DTOs Phase-35-lifted to `dristi-common/contract/casemanagement/` + `@ApplicationModule(displayName="Casemanagement")` boundary (no `*Api` initially — no incoming edges from peer subdomains); Rule 32 first pass on the 3 Phase-5-flagged utils — `CaseUtil → CaseApi.search`, `TaskManagementUtil → TaskmanagementApi.search`; Rule 41 `@Component("casemanagementCacheUtil")`. **C3** `b69be7650` = Rule 32 second pass on REST utils Phase 5 missed — exposed `ApplicationApi` (first cross-subdomain caller); `EvidenceUtil → EvidenceApi`, `OrderUtil → OrderApi`, `ApplicationUtil → ApplicationApi`. **C4** `78df71bdc` = Rule 32 final sweep — **re-introduced `TaskApi`** (reverted in `a89087936` for the cases↔task↔order cycle-break, safe here because casemanagement has zero incoming edges — verified across done/in-flight/pending services); exposed `CtcApi.search` + `DigitalizeddocumentsApi.search`; converted `TaskUtil`/`CtcUtil.isPartyToCase`/`DigitalizedDocumentUtil`/`CaseBundleService`+`CaseBundleIndexBuilderService` (via `fetchCaseAsMap()` adapter)/`OrderSearchService`/`TaskSearchService`; Rule 36 test rewrites on `OrderSearchServiceTest`+`TaskSearchServiceTest`. **C5 (PR #102 absorbed)** `d654c8644` = added mdms url (`Configuration.mdmsHost` default + `application-local.yml` entry). **C6** `baff7a956` = **first PR to exercise Rule 42 in the post-merge slot** — stripped 21 dead `@Value` fields (egov.user.* / egov.idgen.* / egov.workflow.* / egov.hrms.* / egov.url.shortner.* / sms / file / preview / mdms.kafka / casemanagement.delay.time) + extended `SERVICE_DEAD_KEYS["casemanagement"]` so Pipeline 5 regen won't re-inject. Rule 40 mutation scan clean (no `RequestInfo` mutations). Deferred: `CtcUtil.updateCtcApplication` (Rule 35 cross-subdomain write); `MdmsV2Util` + `SummonsOrderPdfUtil` (Rule 17 egov platform). |
