...
| `e-sign-svc` + `esign-interceptor` | 68 | in-progress | `integration` | `esign` | PR 1 = `e-sign-svc` pipeline migration to `domain-integration/esign` (structural lift only — interceptor port follows in PR 2). C1 `66c930c1c`: 33 main + 11 test files; Rule 26 `ResponseInfoFactory` static→instance adapt in `EsignApiController`; Rule 41 `MainConfiguration` qualified as `esignMainConfiguration`; Rule 25 dropped `mockito-core:3.12.4` override in `domain-integration/pom.xml` so it inherits parent `5.7.0` (required to mock JDK-module `javax.xml.crypto.dsig.XMLSignatureFactory`); pipeline Gate 5 cross-platform fix (`shutil.which("mvn")` so Windows `mvn.cmd` and Unix `mvn` both work without `shell=True`); `SERVICE_DEAD_KEYS` extended for `case`+`order` to suppress dead `egov.advocate.*` keys from regens (matches `483b71f8c` pattern). C2 `bbfbc782b`: 12 contract DTOs Phase-35-lifted to `dristi-common/contract/esign/`; orphan `File` shadow-imports removed from 3 main + 1 test file (`feedback_lifted_dto_shadowing` memory). 0 Rule 32 conversions in C2 (no intra-DRISTI REST calls); bail-bond + digitalizeddocuments → esign REST→direct deferred (no `EsignApi` exposed yet). FileStoreUtil follow-up Tier 3 PR pending. |
