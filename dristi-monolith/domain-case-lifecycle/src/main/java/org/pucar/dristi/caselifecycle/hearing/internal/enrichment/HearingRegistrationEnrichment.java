...
public class HearingRegistrationEnrichment {
    ...
    @Autowired
    public HearingRegistrationEnrichment(IdgenUtil idgenUtil, Configuration configuration, WorkflowUtil workflowUtil, WorkflowService workflowService) {
    ...
    private void enrichHearingDuration(HearingRequest hearingRequest) {
        ...
        try {
            ...
            List<ProcessInstance> processInstance = workflowService.getProcessInstances(hearingRequest.getRequestInfo(), hearingRequest.getHearing().getTenantId(), hearingRequest.getHearing().getHearingId());
