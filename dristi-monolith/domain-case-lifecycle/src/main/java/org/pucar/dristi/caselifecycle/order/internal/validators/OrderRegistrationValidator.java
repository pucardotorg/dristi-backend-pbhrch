...
public class OrderRegistrationValidator {
    ...
    private void validateMDMSDocumentTypes(OrderRequest orderRequest) {
        Map<String, Map<String, JSONArray>> mdmsRes = mdmsUtil.fetchMdmsData(orderRequest.getRequestInfo(), orderRequest.getOrder().getTenantId(), configuration.getOrderModule(), createMasterDetails());
        String mdmsData;
        try {
            mdmsData = objectMapper.writeValueAsString(Collections.singletonMap("MdmsRes", mdmsRes));
        } catch (Exception e) {
            log.error("Failed to serialize MDMS response for DocumentType validation", e);
            return;
        }
