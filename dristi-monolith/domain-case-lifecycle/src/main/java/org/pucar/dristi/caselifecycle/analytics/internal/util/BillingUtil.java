...
import static org.pucar.dristi.caselifecycle.analytics.internal.config.ServiceConstants.CASE_COURTCASENUMBER_PATH;

@Slf4j
@Component("analyticsBillingUtil")
public class BillingUtil {
    ...
    public String getDemand(String tenantId, String demandId, JSONObject requestInfo) {
        ...
        try {
            Object responseObj = requestRepository.fetchResult(new StringBuilder(url), requestInfo);
            response = responseObj == null ? null : objectMapper.writeValueAsString(responseObj);
        } catch (ServiceCallException | JsonProcessingException e) {
