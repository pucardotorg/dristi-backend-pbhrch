...
public class IdGenUtilTest {
    ...
    public void testGetIdList_Success() {
        ...
        doReturn(Collections.singletonMap("dummyKey", "dummyValue")).when(restRepo).fetchResult(any(StringBuilder.class), any());
    ...
    public void testGetIdList_Exception() {
        ...
        doReturn("http://idgen-host").when(configs).getIdGenHost();
        doReturn("/idgen-path").when(configs).getIdGenPath();
        doThrow(new RuntimeException("Error")).when(restRepo).fetchResult(any(StringBuilder.class), any());
